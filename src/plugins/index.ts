...
import type { HookHandler, HookEvent } from '../hooks';
...
const PLUGINS_DIR = join(homedir(), '.flipagent', 'plugins');
const PLUGIN_SETTINGS_FILE = join(homedir(), '.flipagent', 'plugin-settings.json');
...
export interface PluginService {
  ...
  destroy(): void;
  /** Subscribe to events */
  on<K extends keyof PluginServiceEvents>(
    event: K,
    fn: (...args: PluginServiceEvents[K]) => void
  ): void;
  off<K extends keyof PluginServiceEvents>(
    event: K,
    fn: (...args: PluginServiceEvents[K]) => void
  ): void;
}
...
function loadAllSettings(): Record<string, Record<string, unknown>> {
  try {
    ...
    logger.warn({ error: err, path: PLUGIN_SETTINGS_FILE }, 'Failed to load plugin settings');
  }
  return {};
}
...
export function createPluginService(): PluginService {
  ...
  function loadStorage(pluginId: string): Record<string, unknown> {
    ...
    try {
      ...
      logger.warn({ error: err, pluginId, storageFile }, 'Failed to load plugin storage');
    }
    return {};
  }
  ...
  const service: PluginService = {
    ...
    async executeCommand(name, args, ctx) {
      for (const [_pluginId, registered] of Array.from(plugins)) {
    ...
    listCommands() {
      ...
      for (const [pluginId, registered] of Array.from(plugins)) {
        if (registered.state !== 'enabled') continue;

        for (const [name, cmd] of Array.from(registered.commands)) {
    ...
    getTools() {
      ...
      for (const [_pluginId, registered] of Array.from(plugins)) {
        if (registered.state !== 'enabled') continue;
        tools.push(...Array.from(registered.tools.values()));
      }
    ...
    async loadFromDirectory(dir = PLUGINS_DIR) {
      ...
      try {
        ...
        logger.warn({ error: err, dir }, 'Failed to read plugins directory');
      }
    ...
    off(event, fn) {
      emitter.off(event, fn as (...args: unknown[]) => void);
    },
  };
  ...
  onEnable(ctx) {
    ctx.registerCommand(
      ...
      (args, cmdCtx) => {
        ...
        const reminders =
          ctx.storage.get<Array<{ userId: string; message: string; time: number }>>('reminders') ??
          [];
        reminders.push({
