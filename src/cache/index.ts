...
/**
 * Product cache - for caching product data lookups
 */
...
export function createProductCache<V = unknown>(
  options: Omit<CacheOptions<ProductCacheKey, V>, 'keySerializer'> = {}
): LRUCache<ProductCacheKey, V> {
  return createCache<ProductCacheKey, V>({
    name: 'product-cache',
    maxSize: 500,
    defaultTtl: 30000, // 30 seconds - prices change frequently
    ...options,
    keySerializer: (key) => `${key.platform || '*'}:${key.productId}`,
  });
}
