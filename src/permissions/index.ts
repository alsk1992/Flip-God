/**
 * Permissions Module - Exec approvals, command allowlisting, and sandbox enforcement
 *
 * Features:
...
export const TOOL_GROUPS: Record<string, string[]> = {
  ...
  'group:scraping': ['scrape_product', 'scrape_search', 'scrape_price'],
};

/** Built-in tool profiles */
export const TOOL_PROFILES: Record<ToolProfile, string[]> = {
  minimal: ['session_status'],
  coding: ['session_status', 'group:fs', 'group:runtime', 'group:sessions', 'group:memory'],
  messaging: ['session_status', 'group:messaging', 'group:sessions'],
  full: ['*'],
  custom: [],
};
...
export class ExecApprovalsManager extends EventEmitter {
  ...
  constructor(configPath?: string) {
    super();
    this.configPath =
      configPath || join(homedir(), '.flipagent', 'exec-approvals.json');
    this.pendingPath = join(homedir(), '.flipagent', 'exec-approvals.pending.json');
    this.decisionsPath = join(homedir(), '.flipagent', 'exec-approvals.decisions.json');
    ...
    try {
      if (existsSync(this.decisionsPath)) {
        const data = JSON.parse(
          readFileSync(this.decisionsPath, 'utf-8'),
        ) as ApprovalDecisionsFile;
  ...
  getPendingApprovalsFromDisk(): ApprovalRequest[] {
    ...
    const now = Date.now();
    const active = data.pending.filter(
...
export class ElevatedPermissions {
  ...
  disallow(provider: string, entry: string): void {
    if (this.config.allowFrom[provider]) {
      this.config.allowFrom[provider] = this.config.allowFrom[provider].filter(
        (e) => e !== entry,
      );
    }
  }
