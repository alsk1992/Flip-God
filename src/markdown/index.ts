/**
 * Markdown Module - Platform-specific message formatting
 *
 * Features:
...
export type MarkdownPlatform = 'telegram' | 'discord' | 'slack' | 'whatsapp' | 'plain' | 'html';
...
function toHtml(text: string): string {
  ...
  result = result.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, alt, url) => {
    const safeUrl = sanitizeUrl(url);
    return safeUrl
      ? `<img src="${safeUrl}" alt="${sanitizeHtmlAttr(alt.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>'))}">`
      : alt;
  });
...
export function escape(text: string, platform: MarkdownPlatform): string {
  switch (platform) {
    case 'telegram':
...
export function unescape(text: string, platform: MarkdownPlatform): string {
  switch (platform) {
    case 'telegram':
      return text.replace(/\\([_*\[\]()~`>#+\-=|{}.!\\])/g, '$1');
    case 'html':
