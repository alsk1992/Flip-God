/**
 * Logging Module - Structured JSON logging with file rotation
 *
 * Features:
 * - Structured JSON logging
 * - Log levels (debug, info, warn, error)
 * - File rotation (10MB max, 5 rotations)
...
export function createStructuredLogger(options: LoggerOptions = {}): Logger {
  ...
  if (options.file) {
    if (typeof options.file === 'string') {
      ...
      const logsDir = join(homedir(), '.flipagent', 'logs');
      if (!existsSync(logsDir)) {
        mkdirSync(logsDir, { recursive: true });
      }
      filePath = join(logsDir, 'flipagent.log');
    }
  }
