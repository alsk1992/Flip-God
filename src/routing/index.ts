/**
 * Routing Service - Multi-agent routing with bindings
 *
 * Features:
 * - Multiple specialized agents (main, sourcing, listing, fulfillment)
...
const DEFAULT_AGENTS: AgentDefinition[] = [
  {
    id: 'main',
    name: 'Main Agent',
    description: 'General purpose retail arbitrage assistant',
    enabled: true,
  },
  {
    id: 'sourcing',
    name: 'Sourcing Agent',
    description: 'Specialized in product sourcing and price comparison',
    systemPrompt: `You are a sourcing specialist for retail arbitrage.
Focus on:
- Product search across platforms (Amazon, eBay, Walmart)
...
Be concise and data-driven.`,
  ...
  },
  {
    id: 'listing',
    name: 'Listing Agent',
    description: 'Specialized in product listing creation and optimization',
    systemPrompt: `You are a listing specialist for retail arbitrage.
Focus on:
- Creating optimized product listings
- Title and description writing
- Pricing strategy
- Category and keyword selection
Be thorough and market-aware.`,
    enabled: true,
  },
  {
    id: 'fulfillment',
    name: 'Fulfillment Agent',
    description: 'Specialized in order fulfillment and shipping',
    systemPrompt: `You are a fulfillment specialist for retail arbitrage.
Focus on:
- Order tracking and processing
- Shipping cost optimization
- Delivery status monitoring
- Returns and customer issues
Be precise about timelines and costs.`,
    enabled: true,
  },
];
...
const DEFAULT_BINDINGS: Binding[] = [
  ...
  // Keyword bindings
  {
    id: 'kw-source',
    type: 'keyword',
    pattern: 'find deals|source|search products|compare prices',
    agentId: 'sourcing',
    priority: 50,
    enabled: true,
  },
  {
    id: 'kw-listing',
    type: 'keyword',
    pattern: 'create listing|list item|optimize title|write description',
    agentId: 'listing',
    priority: 50,
    enabled: true,
  },
  {
    id: 'kw-fulfill',
    type: 'keyword',
    pattern: 'ship order|track package|fulfillment|delivery status',
    agentId: 'fulfillment',
    priority: 40,
    enabled: true,
  },
