...
const PERSISTENCE_DIR = join(homedir(), '.flipagent', 'subagents');
...
export function createSubagentManager(): SubagentManager {
  ...
  const defaultToolExecutor: ToolExecutor = async (toolName) => {
  ...
  async function executeAgentLoop(
    ...
    const model = state.config.model || 'claude-sonnet-4-5-20250929';
    ...
    try {
      ...
      while (state.turn < maxTurns && state.status === 'running') {
        ...
        state.cost.totalCost = calculateCost(
          model,
      ...
      if (
        ...
        state.retryCount < (state.config.maxRetries || 3)
      ) {
        state.retryCount++;
